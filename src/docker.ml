...
(* String list to string with sep*)
...
let read_headers fn_name buf fd =
  print_endline "read headers inside";
  let headers = ref [] in
  ...
  while !continue do
    ...
    if r > 0 then
      ...
      if i < 0 then Buffer.add_subbytes buf b (* (Bytes.to_string b) *) 0 r
        ...
        let i = ref 0 in
        while
          i := index_CRLF b ~pos:!pos ~len:!len;
          !i > !pos
        do
          let h_len = !i - !pos in
...
let get fn_name addr url query =
  print_endline " get inside";
  let buf = Buffer.create 256 in
  Buffer.add_string buf "GET /v1.42";
...
let response_of_get fn_name addr url query =
  print_endline "res of get inside";
...
let[@inline] post_header url query =
  let buf = Buffer.create 4096 in
  Buffer.add_string buf "POST /v1.42";
  Buffer.add_string buf url;
  Buffer.add_encoded_query buf query;
  Buffer.add_string buf Docker_config.http11_header;
  buf
...
let message_of_body body =
  ...
  | `Assoc l -> (
      try
        match List.assoc "message" l with
        | `String m -> m
        | j -> Json.to_string j
      with Not_found -> body)
  | _ -> body
...
module Stream = struct
  ...
  type t = {
    ...
    buf : Bytes.t;
    mutable i0 : int;
    mutable i1 : int; (* Partially decoded stream *)
  }
  ...
  let create buffer fd =
    ...
    { fd; out = Unix.out_channel_of_descr fd; buf; i0 = 0; i1 = 0 }

  let out st = st.out
  let shutdown st = Unix.shutdown st.fd Unix.SHUTDOWN_SEND
  ...
  let fill_if_needed st ~timeout =
    if st.i0 >= st.i1 then (
      st.i0 <- 0;
      st.i1 <- 0;
      ignore (if timeout < 0. then fill_unbounded_wait st else fill st ~timeout))
  ...
  let read_header st ~timeout =
    ...
    if st.i0 + 8 > st.i1 then
      ...
      let typ =
        match Bytes.get st.buf st.i0 with
        | '\000' -> STDIN
        | '\001' -> STDOUT
        | '\002' -> STDERR
        | _ -> raise (Error ("Docker.Stream.read", "invalid STREAM_TYPE"))
      in
      ...
      if Sys.word_size = 32 && (size1 lsr 7 = 1 || len > Sys.max_string_length)
      then
        failwith
          "Docker.Stream.read: payload exceeds max string length (32 bits)";
      st.i0 <- st.i0 + 8;
      (* 8 bytes processed *)
      if len = 0 then
  ...
  let read_all st =
    ...
    while !continue do
      ...
      if String.length b = 0 then continue := false else l := r :: !l
    done;
    List.rev !l
...
module Container = struct
  ...
  let container_of_json (c : Json.t) =
    ...
    | `Assoc l ->
        ...
        List.iter update l;
        {
          ...
          size_root_fs =
            (if !size_root_fs < 0 then None else Some !size_root_fs);
        }
    | _ ->
        raise
          (Error
             ("Docker.Container.list", "Invalid container: " ^ Json.to_string c))
  ...
  let list ?(addr = !default_addr) ?(all = false) ?limit ?(size = false) ?before
      ?exited ?health ?name ?since ?status ?volume () =
    print_endline "list method called";
    let q = if all then [ ("all", "1") ] else [] in
    let q =
      match limit with Some l -> ("limit", string_of_int l) :: q | None -> q
    in
    ...
    in
    if status >= 400 then raise (Invalid_argument "Docker.Container.list");
    match Json.from_string body with
    | `List l -> List.map container_of_json l
    | _ ->
        raise
          (Error ("Docker.Container.list", "response not a JSON list: " ^ body))
  ...
  let host ?(cpu_shares = 0) ?(memory = 0) ?(cgroup_parent = "")
      ?(blk_io_weight = -1) ?(cpu_period = 0) ?(memory_swap = -1) ?(binds = [])
      ?(network_mode = "bridge") ?(policy = `None) () =
    {
      cpu_shares;
      memory;
      cgroup_parent;
  ...
  let restart_policy name count =
    ( "RestartPolicy",
      `Assoc [ ("Name", `String name); ("MaximumRetryCount", `Int count) ] )

  let create ?(addr = !default_addr) ?(hostname = "") ?(domainname = "")
      ?(user = "") ?(stdin = false) ?(stdout = true) ?(stderr = true)
      ?(open_stdin = false) ?(stdin_once = false) ?(env = []) ?(workingdir = "")
      ?(networking = false) ?(host = default_host) ?name image cmd =
    (*** Host Config *)
    let host_config =
    ...
    let memory, memory_swap =
      ...
      else (host.memory, Int.max host.memory host.memory_swap)
    in
    let host_config =
      ("Memory", `Int memory)
    ...
    let json : Json.t =
      `Assoc
        [
          ...
          (* StopTimeout *)
          (* Shell *)
          ("HostConfig", `Assoc host_config) (* NetworkingConfig *);
        ]
    in
    let query_params =
      match name with
      | Some name ->
          if name_is_not_allowed name then
            invalid_arg
              (Printf.sprintf
                 "Docker.Container.create: container name %S is not allowed"
                 name);
          [ ("name", name) ]
      | None -> []
    in
    let status, _, body =
      response_of_post "Docker.Container.create" addr "/containers/create"
        query_params (Some json)
    in
    (if status >= 409 then raise (Failure ("Docker.Container.create", body))
      ...
      | m -> (
          ...
          with _ -> raise (Failure ("Docker.Container.create", m)))
      | exception Yojson.Json_error _ ->
          raise
            (Server_error (Printf.sprintf "body %S contains no message" body)));
    (* Extract ID *)
    match Json.from_string body with
    | `Assoc l -> (
  ...
  let change_of_json = function
    | `Assoc c -> (
        try
          let path = List.assoc "Path" c in
          let kind =
            match List.assoc "Kind" c with
            | `Int 0 -> `Modified
            | `Int 1 -> `Added
            | `Int 2 -> `Deleted
            | j ->
                raise
                  (Error
                     ( "Docker.Container.changes",
                       "Invalid kind:" ^ Json.to_string j ))
          in
          (string_of_json "Docker.Container.changes" path, kind)
        with Not_found ->
          raise (Error ("Docker.Container.changes", "Invalid change object")))
    | j ->
        raise
          (Error
             ( "Docker.Container.changes",
  ...
  let changes ?(addr = !default_addr) id =
    ...
    | _ ->
        raise (Error ("Docker.Container.changes", "Invalid response: " ^ body))
  ...
  let start ?(addr = !default_addr) ?(detach_keys = "") id =
    ...
    let status, _, body =
      response_of_post "Docker.Container.start" addr path q None
    in
    if status >= 404 then raise (No_such_container id);
    if status >= 400 then
  ...
  let stop ?(addr = !default_addr) ?wait id =
    let q =
      match wait with None -> [] | Some t -> [ ("t", string_of_int t) ]
    in
    let path = "/containers/" ^ id ^ "/stop" in
    let status =
      status_response_of_post "Docker.Container.stop" addr path q None ~id
    in
    if status >= 304 then
  ...
  let wait ?(addr = !default_addr) id =
    ...
    let _, _, body =
      response_of_post "Docker.Container.wait" addr path [] None
    in
    match Json.from_string body with
    | `Assoc l -> (
        try
          match List.assoc "StatusCode" l with
          | `Int s -> s
  ...
  let rm ?(addr = !default_addr) ?(volumes = false) ?(force = false)
      ?(link = false) id =
    let q =
      [
        ("v", string_of_bool volumes);
        ("force", string_of_bool force);
        ("link", string_of_bool link);
      ]
    in
    let path = "/containers/" ^ id in
    let status, _, body =
      response_of_delete "Docker.Container.rm" addr path q
    in
    if status >= 409 then
  ...
  let kill ?(addr = !default_addr) ?signal id =
    let q =
      match signal with Some s -> [ ("signal", string_of_int s) ] | None -> []
    in
  ...
  let attach ?(addr = !default_addr) ?(stdin = false) ?(stdout = false)
      ?(stderr = false) id which =
    let logs, stream =
      match which with
      | `Logs -> ("true", "false")
      | `Stream -> ("false", "true")
      | `Logs_and_stream -> ("true", "true")
    in
    let q =
      [
        ("logs", logs);
  ...
  module Exec = struct
    ...
    let create ?(addr = !default_addr) ?(stdin = false) ?(stdout = true)
      ...
      in
      let json =
        match detach_keys with
        | Some d -> ("DetachKeys", `String d) :: json
        | None -> json
      in
      let json =
        match user with Some u -> ("User", `String u) :: json | None -> json
      in
      let path = "/containers/" ^ container ^ "/exec" in
      let status, _, body =
        response_of_post "Docker.Container.Exec.create" addr path []
          (Some (`Assoc json))
      in
      (if status >= 400 then
       (* Try to extract the container ID. *)
      ...
      | `Assoc l -> (
          ...
          with _ ->
            raise (Error ("Docker.Containers.Exec.create", "No ID returned")))
      | _ ->
          raise
            (Error
               ( "Docker.Container.Exec.create",
                 "Response must be an association list: " ^ body ))

    let start ?(addr = !default_addr) exec_id =
      let json = `Assoc [ ("Detach", `Bool false); ("Tty", `Bool false) ] in
      ...
      if status >= 409 then (
        ...
        raise
          (Failure
             ("Docker.Container.Exec.start", "Container is stopped or paused")))
      else if status >= 400 then (
        Unix.close fd;
...
module Image = struct
  ...
  let image_of_json (img : Json.t) =
    ...
    | _ ->
        raise
          (Error ("Docker.Images.list", "Invalid image: " ^ Json.to_string img))

  let list ?(addr = !default_addr) ?(all = false) () =
    let q = [ ("all", string_of_bool all) ] in
    let _, _, body =
      response_of_get "Docker.Images.list" addr "/images/json" q
    in
    match Json.from_string body with
    | `List l -> List.map image_of_json l
    | _ ->
        raise
          (Error ("Docker.Images.list", "Response must be a JSON list: " ^ body))
  ...
  let create ?(addr = !default_addr) ?(platform = "") from =
    ...
    let status =
      ...
      | Image im ->
          let q =
            ("fromImage", im.name) :: ("repo", im.repo) :: ("tag", im.tag) :: q
          in
          let status, _, _ =
            response_of_post "Docker.Image.create" addr "/images/create" q None
          in
          status
      | Src url ->
          if url = "-" then
    ...
    if status >= 404 then
      raise
        (Failure
           ("Docker.Image.create", "repository does not exist or no read access"))

  let from_image ?(repo = "") ?(tag = "") name = Image { name; repo; tag }
end
